...
<<gb, echo = FALSE, message=FALSE,warning=FALSE, fig.cap="Slave trade routes run by British ships, 1562-1809. Color represents the century in which the ship departed Africa with slaves.", fig.pos='h'>>=
...
<<portugal, echo = FALSE, eval = FALSE, message=FALSE,warning=FALSE, fig.cap="Slave trade routes run by Portuguese ships from 1525-1854.", fig.pos='h'>>=
...
mp + geom_net(data= usa_death_data, layout = NULL, 
              ealpha = .1, arrowgap = 1, directed = TRUE,color = "#ff7f00",
              aes(x=longitude, y = latitude, from_id = mjbyptimp, to_id=mjslptimp,
                   size = slaximp, arrowsize = arrowlength)) +
...
<<first_years, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="All ships arriving in the Americas in the first 75 years of the slave trade, from 1525-1600. Port name is labeled.", fig.pos='h'>>=
...
ggplot() + geom_net(data= spain, directed = TRUE, layout = 'fruchtermanreingold',ealpha = .2, arrowgap = .01, vjust = -.2,
          aes(from_id = mjbyptimp, to_id=mjslptimp, arrowsize= arrowsize, label = mjbyptimp, linewidth = slaximp/100)) + lims(x = c(-.1,1.1), y = c(-.1,1.1))
@

\section*{Conclusion}

After exploring the Trans-Atlantic Slave Trade Database, I have a much deeper understanding of the slave trade any history class has ever given me. 
