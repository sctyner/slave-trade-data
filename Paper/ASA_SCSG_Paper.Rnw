...
The required aesthetic options are 
  ...
  \begin{itemize}
  \item \texttt{x} and \texttt{y}: if \texttt{layout = NULL}, \texttt{x} and \texttt{y} set the locations of the nodes (e.g. longitude and latitude)  \st{XXX add more? XXX}
  \end{itemize}
...
<<deaths,echo = FALSE, eval = FALSE, message=FALSE,warning=FALSE, fig.cap="Visualizing deaths of captives during trip. The wider the edge, the more deaths there were on the voyage.">>=
...
\par Next I explore the evolution of the slave trade over time. In the first twenty-five years of slaves being sold in the Americas, from 1525-1550, there were only two voyages arriving. Both were Portuguese ships carrying 300 slaves from S\~{a}o Tom\'{e} in the Gulf of Guinea to the Caribbean. In \autoref{fig:firstyears} the first 75 years of voyages, from 1525 to 1600 are shown. There are 98 voyages in these first years, taking 20,951 Africans from their homes and selling 15,322 of them into slavery, with 5,629 dying during the voyages. The only countries participating in in the slave trade in these first years are Spain, Portugal, and Great Britain, and they are only embarking and disembarking from 20 different ports: ten in Africa and ten in the Americas. 
...
\par I also consider the last years of the slave trade. The last year of the slave trade was 1864, and only one ship sold slaves in the Americas that year: a French ship with 673 Africans from the Congo River area who were sold into slavery in Cuba despite the fact that the slave trade was outlawed in France over 40 years prior in 1818. The last country in the dataset to abolish the slave trade was Portugal in 1836, yet the slave trade continued for almost two decades more, albeit at a much slower pace. The American ships that were still trading after 1807 got around the law, which actually only prohibited import of new slaves to the United States, by still participating in the slave trade and mostly avoiding ports in US territory. This is shown in \autoref{fig:us1808}, with the years of disembarkation and embarkation of slaves surrounding the ports. There is some over-plotting of the labels, but we can see that most of the slave trade routes still being run by US ships after the ban end with slaves being sold in Brazil and in Cuba. 
...
<<postban, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="All US ships still participating in the slave trade after its ban in the US in 1807.", fig.pos='h'>>=
