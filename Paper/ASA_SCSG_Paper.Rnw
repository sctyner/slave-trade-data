...
mp2 <- ggplot() + myborders(xlim= c(-105,75), ylim = c(-25,30))
mp2 + geom_net(data= firstyears75, layout = NULL, ealpha = .4, arrowgap = 1, 
...
mp + geom_net(data= postban, layout = NULL, ealpha = .4, arrowgap = 1, 
  ...
  scale_color_brewer(palette = 'Set1', name = "Ship's Nation") +
  theme(legend.position = 'bottom')+
  xlab("Longitude") + ylab("Latitude") #+ lims(x = c(-100,75), y = c(-50,50)) 
@
