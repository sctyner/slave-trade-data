...
<<slaves_size_pts, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Size of points represents the number of slaves embarked and disembarked on each voyage.">>=
  from_to$arrowlength <- 10
  from_to2 <- from_to
  from_to2[,"mjbyptimp"] <- from_to[,"mjslptimp"]
  from_to2[,"mjslptimp"] <- from_to[,"mjbyptimp"]
  from_to2$slaximp <- from_to$slamimp
  from_to2$arrowlength <- 0
  from_to3 <- rbind(from_to, from_to2)
  nodes1 <- unique(as.character(from_to$mjbyptimp))
  nodes2 <- unique(as.character(from_to$mjslptimp))
  nodes <- unique(c(nodes1, nodes2))
  node_idx <- which(ports$name %in% nodes)
  ports <- ports[node_idx,]
  size_node_data <- merge(from_to3, ports, by.x = "mjbyptimp", by.y = "name", all = T)

mp + geom_net(data= size_node_data, layout = NULL, 
              ealpha = .15, arrowgap = 1, directed = TRUE, 
              aes(x=longitude, y = latitude, from_id = mjbyptimp, to_id=mjslptimp,
                  color = natinimp, size = slaximp, arrowsize = arrowlength/10)) +
  theme(legend.position = 'bottom') + lims(x = c(-105,75), y = c(-50,50)) 
@
