...
slavenet_all_arr <- slavenet_all %>% dplyr::arrange(mjbyptimp)
...
rep(unit(c(10,0),'points'), nrow(slavenet_all))[row.match(slavenet_all_arr[,c('voyageid','mjbyptimp')], slavenet_all_arr2[,c('voyageid','mjbyptimp')])]
...
fix_arrows4 <- arrow(angle = rep(30, nrow(slavenet_all)),
                     length = rep(unit(c(10,0),'points'), nrow(slavenet_all))[row.match(slavenet_all_arr[,c('voyageid','mjbyptimp')], slavenet_all_arr2[,c('voyageid','mjbyptimp')])]
,
                     ends = rep("last", nrow(slavenet_all)),
                     type = rep("closed", nrow(slavenet_all)))


fix_arrows5 <- arrow(angle = rep(30, nrow(slavenet_all)),
                     length = unit(slavenet_all_plsarrow$arrowlength,'points'),
                     ends = rep("last", nrow(slavenet_all)),
                     type = rep("closed", nrow(slavenet_all)))
...
mp + geom_net(data= slavenet_all_plsarrow, layout = NULL, 
              ealpha = .15, arrowgap = 1, directed = TRUE, 
              aes(x=longitude, y = latitude, from_id = mjbyptimp, to_id=mjslptimp,
                  color = natinimp, arrowsize = arrowlength/10)) +
  theme(legend.position = 'bottom') + lims(x = c(-105,75), y = c(-50,50)) + theme_net()

mp + geom_net(data= create_net_data(edge_data = from_to, node_data = ports, 
                                     variable = NULL, value = NULL, from_id = "mjbyptimp" , 
                                     to_id = "mjslptimp", node_id = "name"), layout = NULL, 
              ealpha = .15, arrowgap = 1, directed = TRUE, ecolour = 'black',
              #arrow = fix_arrows4,
              aes(x=longitude, y = latitude, from_id = mjbyptimp, to_id=mjslptimp,
                  color = natinimp)) +
  theme(legend.position = 'bottom') + lims(x = c(-105,75), y = c(-50,50)) + theme_net()
...
slavenet_all_plsarrow <- create_net_data3(edge_data = from_to, node_data = ports, variable = NULL, value = NULL, from_id = "mjbyptimp" , to_id = "mjslptimp", node_id = "name")
head(slavenet_all_plsarrow)

table(slavenet_all_plsarrow$arrowlength)
